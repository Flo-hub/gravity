        ...
        for p in neighbors:
            if point.distance_to_proj(p) < point.distance_to(neighbors[k - 1]):
                ...
                for q in self.points:
                    if q.h > p.h and (
                            q.path[:p.h - 1] == point.path[:p.h - 1] and q.path[p.h] == (point.path[p.h] + 1) % 2):
                        if len(neighbors) == k:
...
def classify(name, l = 5):
    ...
    for k in range(1, 201):
        ...
        for r in range(l - 1):
            ...
            for w in data_set[r * l1:(r + 1) * l1]:
                value = 0
                for p in trees[r].neighbors_of(Point(w[1:], w[0]), k):
                    value += p.value
                if sign(value) != w[0]:
                    R += 1
        ...
        for w in data_set[(l - 1) * l1:]:
            value = 0
            for p in trees[l - 1].neighbors_of(Point(w[1:], w[0]), k):
                value += p.value
                if sign(value) != w[0]:
                    R += 1
                    # values.append(sign(value))
    ...
    for v in test_set:
        ...
        for r in range(l):
            value = 0
            for p in trees[r].neighbors_of(Point(v[1:], v[0]), K):
                value += p.value
                value = sign(value)
